  ...
  run: async (client, interaction) => {
    try {
      ...
      try {
        ...
        return interaction.editReply({ content: `❌ Ko có kết quả` }).catch(e => { });
      }

      if (!res || !res.length || !res.length > 1) return interaction.reply({ content: `❌ Ko có kết quả`, ephemeral: true }).catch(e => { });
          ...
          switch (button.customId) {
            ...
            default: {
              ...
              try {
                ...
                await interaction.editReply({ content: `❌ Ko có kết quả!`, ephemeral: true }).catch(e => { });
              }
              return collector.stop();
            }
          }
